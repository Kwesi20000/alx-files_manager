...
class AuthController {
  /**
   * Class should sign-in the user by generating a new authentication token
   *
   * By using the header Authorization and the technique of the Basic auth
   * (Base64 of the <email>:<password>), find user associated to this email
   * and with this password
   * If no user has been found, returns an error Unauthorized with a status code 401
   * Otherwise:
   * Generates a random string (using uuidv4) as token
   * Create a key: the auth_<token>
   ...
   *
   * this retrieves the user based on the token:
